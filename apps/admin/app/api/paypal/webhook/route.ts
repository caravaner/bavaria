...
async function handleCaptureCompleted(event: PayPalWebhookEvent) {
  ...
  const reconciled = await db.order.update({
    where: { id: order.id },
    data: { status: "CAPTURED", capturedAt: new Date() },
  });
  console.log("[webhook] reconciled Order to CAPTURED", { orderId: order.id });
  // Send the guest confirmation + owner notice if the capture handler never
  // did. Both dedupeKey'd, so this is a no-op when the emails already went out.
  await Promise.all([
    sendOrderConfirmation(reconciled),
    sendOwnerBookingNotice(reconciled),
  ]);
}
